...
| Property       | Default  | Description                                                   |
| -------------- | -------- | ------------------------------------------------------------- |
| format-version | 2        | Table's format version (can be 1 or 2) as defined in the [Spec](https://iceberg.apache.org/spec/#format-versioning). Defaults to 2 since version 1.4.0. |
...
| metrics-reporter-impl | org.apache.iceberg.metrics.LoggingMetricsReporter | Custom `MetricsReporter` implementation to use in a catalog. See the [Metrics reporting](https://iceberg.apache.org/docs/1.5.2/metrics-reporting/) section for additional details |
...
Spark uses its session properties as catalog properties, see more details in the [Spark configuration]($Flink-Configuration#catalog-configuration) section.
Flink passes in catalog properties through `CREATE CATALOG` statement, see more details in the [Flink]($Flink-Getting-Started#adding-catalogs) section.
...
| lock.table                        | null               | an auxiliary table for locking, such as in [AWS DynamoDB lock manager]($AWS#dynamodb-lock-manager) |
