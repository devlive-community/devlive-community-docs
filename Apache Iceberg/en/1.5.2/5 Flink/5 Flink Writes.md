...
For more information on branches please refer to [branches]($Branching-And-Tagging).
```java
FlinkSink.forRowData(input)
    .tableLoader(tableLoader)
...
store uncommitted data as temporary files. Therefore, [expiring snapshots]($Maintenance#expire-snapshots)
and [deleting orphan files]($Maintenance#delete-orphan-files) could possibly corrupt
