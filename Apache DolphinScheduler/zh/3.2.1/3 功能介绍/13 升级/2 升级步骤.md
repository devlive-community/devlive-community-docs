...
在升级之前，您应该检查 [incompatible change]($Upgrade-Incompatible)，因为一些不兼容的更改可能会破坏您当前的功能。
...
根据你部署方式停止 dolphinscheduler 的所有服务，如果你是通过 [集群部署]($Deployment-Cluster) 来部署你的 dolphinscheduler 的话，可以通过 `sh ./script/stop-all.sh` 停止全部服务。
...
- 伪集群部署请参照[伪集群部署(Pseudo-Cluster)]($Deployment-Pseudo-Cluster)中的 `修改相关配置`
- 集群部署请参照[集群部署(Cluster)]($Deployment-Cluster)中的 `修改相关配置`
