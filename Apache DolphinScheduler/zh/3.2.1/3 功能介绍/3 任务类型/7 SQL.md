...
- 工具栏中拖动 <img src="https://dolphinscheduler.apache.org/img/tasks/icons/sql.png" width="25"/> 到画板中，选择需要连接的数据源，即可完成创建。
...
**注意**：sql任务组件的hive应用是基于JDBC去调用，SQL statement 不支持多行执行，请注意不要在语句末尾使用';'。如果要执行多行语句请使用[Hive-Cli]($Task-Hive-CLI)任务。

![hive-sql](https://dolphinscheduler.apache.org/img/tasks/demo/hive-sql.png)
...
![hive-sql](https://dolphinscheduler.apache.org/img/tasks/demo/hive-result.png)

### 使用前置sql和后置sql示例

在前置sql中执行建表操作，在sql语句中执行操作，在后置sql中执行清理操作。

![pre_post_sql](https://dolphinscheduler.apache.org/img/tasks/demo/pre_post_sql.png)
