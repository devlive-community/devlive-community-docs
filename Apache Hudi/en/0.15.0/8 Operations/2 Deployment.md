...
[Hudi Streamer]($Using-Spark#hudi-streamer) is the standalone utility to incrementally pull upstream changes
...
New Hudi releases are listed on the [releases page](https://hudi.apache.org/releases/download), with detailed notes which list all the changes, with highlights in each release.
...
For more details, refer to the detailed [migration guide]($Bootstrapping). In the future, we will be supporting seamless zero-copy bootstrap of existing tables with all the upsert/incremental query capabilities fully supported.
