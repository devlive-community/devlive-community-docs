...
Hudi maintains a scalable [metadata]($Metadata-Table) that has some auxiliary data about the table.
...
First, we will generate a continuous workload. In the below example, we are going to start a [Hudi Streamer]($Using-Spark#hudi-streamer) which will continuously write data
...
metadata table can be explored [here]($Metadata-Table#metadata-table-indices) along with [configs]($Metadata-Table#enable-hudi-metadata-table-and-multi-modal-index-in-write-side)
to enable them. The full set of metadata configurations can be explored [here]($All-Configurations#Metadata-Configs).
