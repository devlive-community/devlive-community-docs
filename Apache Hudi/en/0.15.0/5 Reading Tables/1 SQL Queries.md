...
## Spark SQL
The Spark [quickstart]($Spark-Quick-Start) provides a good overview of how to use Spark SQL to query Hudi tables. This section will go into more advanced configurations and functionalities.
...
Please refer to [configurations]($Basic-Configurations) section for the important configuration options.
...
relying on the custom Hudi input formats like Hive. Typically, notebook users and Flink SQL CLI users leverage flink sql for querying Hudi tables. Please add hudi-flink-bundle as described in the [Flink Quickstart]($Flink-Quick-Start).
...
>  The most recent release of Athena that supports querying Hudi 0.14.0 tables has a bug that causes _ro query to return 0 records, and occasionally _rt the query to fail with class cast exception.
